...
//	2. What are all the unique cities where the traders work?
		...
		List<Trader> getAllTraders = transactionList.stream().map(transaction -> transaction.getTrader()).distinct()
				.collect(Collectors.toList());

		List<String> getAllTrasersCity = getAllTraders.stream().map(trader -> trader.getCity()).distinct()
				.collect(Collectors.toList());
...
//	3. Find all traders from Delhi and sort them by name.
		...
		List<Trader> getAllTradersFromDelhi = getAllTraders.stream().filter(city -> city.getCity().equals("delhi"))
...
//	5. Are any traders based in Jaipur?
		...
		boolean anyTraderBasedOnJaipur = getAllTraders.stream().anyMatch(trader -> trader.getCity().equals("jaipur"));
...
//	6. Print all transactions values from the traders living in delhi
		...
		List<Integer> printValuLivingInDelhi = transactionList.stream()
				.filter(city -> city.getTrader().getCity().equals("delhi")).map(value -> value.getValue())
				.collect(Collectors.toList());
...
//	7. Whats the highest value of all the transactions?
		...
		OptionalInt maxValueFromTransactions = transactionList.stream().mapToInt(value -> value.getValue()).max();
...
//	8. Find the transaction with the smallest value
		...
		List<Transaction> transactionWithSmallestValue = transactionList.stream().limit(1)
